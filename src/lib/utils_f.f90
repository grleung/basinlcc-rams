!##############################################################################
Subroutine trunc_4d_vars (varn,nzg,nxp,nyp,npatch,a)

implicit none

character(len=32) :: varn
integer :: nzg,npatch,nxp,nyp
real :: a(nzg,nxp,nyp,npatch)

if(trim(varn)=='SOIL_WATER')then
a = anint(a*1e6)/1e6
elseif(trim(varn)=='SOIL_ENERGY')then
a = anint(a*1e2)/1e2
endif

return
END SUBROUTINE trunc_4d_vars
!##############################################################################
Subroutine trunc_3d_leaf_vars (varn,nxp,nyp,npatch,a)

implicit none

character(len=32) :: varn
integer :: npatch,nxp,nyp
real :: a(nxp,nyp,npatch)

if(trim(varn)=='STOM_RESIST')then
a = anint(a*1e8)/1e8
elseif(trim(varn)=='VEG_TEMP')then
a = anint(a*1e3)/1e3
elseif(trim(varn)=='VEG_WATER')then
a = anint(a*1e6)/1e6
elseif(trim(varn)=='CAN_TEMP')then
a = anint(a*1e3)/1e3
elseif(trim(varn)=='CAN_RVAP')then
a = anint(a*1e6)/1e6
elseif(trim(varn)=='GROUND_RSAT')then
a = anint(a*1e6)/1e6
elseif(trim(varn)=='GROUND_RVAP')then
a = anint(a*1e6)/1e6
elseif(trim(varn)=='USTAR')then
a = anint(a*1e4)/1e4
elseif(trim(varn)=='TSTAR')then
a = anint(a*1e4)/1e4
elseif(trim(varn)=='RSTAR')then
a = anint(a*1e8)/1e8
endif

return
END SUBROUTINE trunc_3d_leaf_vars
!##############################################################################
...
a = anint(a*1e5)/1e5
elseif(trim(varn)=='FTHRDLW')then
a = anint(a*1e5)/1e5
elseif(trim(varn)=='FTHRDSW')then
a = anint(a*1e5)/1e5
...
elseif(trim(varn)=='PCPRR'.or. &
 ...
 a = anint(a*t_pcpr)/t_pcpr
elseif(trim(varn)=='SFLUX_T') then
 a = anint(a*1e6)/1e6
elseif(trim(varn)=='SFLUX_R') then
 a = anint(a*1e8)/1e8
elseif(trim(varn)=='GLAT' .or. &
      trim(varn)=='GLON')then
 a = anint(a*1e2)/1e2
endif
