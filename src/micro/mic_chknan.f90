...
! GRL added radiation for debugging
use mem_radiate
...
do k = 1,mzp
   ...
   ! GRL 2024-08-28 Checking the FTHRD and FTHRDP values
   if(isnanr(radiate_g(ngrid)%fthrd(k,i,j)))   prtflg=1
   if(isnanr(radiate_g(ngrid)%fthrdp(k,i,j)))   prtflg=1
 ...
 if(prtflg==1)then
    ...
    ! GRL 2024-08-28 Add check on radiation
    print*,'fthrd:         ',radiate_g(ngrid)%fthrd(k,i,j)
    print*,'fthrdp:        ',radiate_g(ngrid)%fthrdp(k,i,j)
